	...
	if (KeGetCurrentIrql()==PASSIVE_LEVEL)
	{
		...
		if (ExAcquireResourceExclusiveLite(&ProcesslistR, TRUE))
		{
			...
			if ((ProcessWatcherOpensHandles) && (WatcherProcess))
			{
				...
				{
					InternalProcessList = ExAllocatePool2(POOL_FLAG_PAGED, sizeof(RTL_GENERIC_TABLE), 'IPLT');
			...
			if (FALSE) //moved till next version
			{
				if (Create)
				{
					...
					tempProcessEntry = ExAllocatePool2(POOL_FLAG_PAGED, sizeof(struct ProcessData), 'PRCS');
