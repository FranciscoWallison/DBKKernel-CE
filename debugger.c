...
void debugger_initialize(void)
{
	...
	Stacks = (PSavedStack*)ExAllocatePool2(POOL_FLAG_NON_PAGED, StackCount * sizeof(PSavedStack), 'STCK');


	int i;
	for (i = 0; i < StackCount; i++)
	{
		Stacks[i] = (PSavedStack)ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(SavedStack), 'SVST');
...
//called in passive mode
{
	if (Stacks)
	{
		...
		newStacks = (PSavedStack*)ExAllocatePool2(POOL_FLAG_NON_PAGED, newStackCount * sizeof(PSavedStack), 'NSTK');
		...
		if (newStacks)
		{
			...
			{
				newStacks[i] = (PSavedStack)ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(SavedStack), 'SVST');
				if (newStacks[i])				
