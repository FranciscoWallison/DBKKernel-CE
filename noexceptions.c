...
BOOL NoExceptions_Enter()
{
	...
	__try
	{
		if (cpustate == NULL)
		{
			...
			cpustate = ExAllocatePool2(POOL_FLAG_NON_PAGED, MaxCPUCount * sizeof(CPUSTATE), 'CPUS');
			...
			if (cpustate)
			{
				...
				{
					cpustate[i].NoExceptionVectorList = ExAllocatePool2(POOL_FLAG_NON_PAGED, 256 * sizeof(INT_VECTOR), 'IVEC');
