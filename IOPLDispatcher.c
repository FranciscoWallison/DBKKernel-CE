...
void mykapc(PKAPC Apc, PKNORMAL_ROUTINE NormalRoutine, PVOID NormalContext, PVOID SystemArgument1, PVOID SystemArgument2)
{
	...
	kApc = ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(KAPC), 'KAPC');
...
*/
{
	...
	BOOL r;
	PVOID buffer;
	buffer = ExAllocatePool2(POOL_FLAG_PAGED, max(nInBufferSize, nOutBufferSize), 'BUF ');
		...
		case IOCTL_CE_ALLOCATEMEM_NONPAGED:
			{
				...
				address = ExAllocatePool2(POOL_FLAG_NON_PAGED, size, 'ADDR');
				*(PUINT64)Irp->AssociatedIrp.SystemBuffer=0;
		...
		case IOCTL_CE_ALLOCATE_MEMORY_FOR_DBVM:
		{
			...
			if (mdl)
			{
				...
				if (pfnlist)
				{
					...
					mi = ExAllocatePool2(POOL_FLAG_PAGED, sizeof(DBVMOffloadMemInfo), 'DBVM');
					if (mi)
					{
						mi->List = ExAllocatePool2(POOL_FLAG_PAGED, sizeof(UINT64) * 32, 'MLST');
						if (mi->List)
						{
							mi->Count = 0;
