...
*/
{
	...
	if (DataBlock)
	{		
		waitblock = ExAllocatePool2(POOL_FLAG_NON_PAGED, MaxDataBlocks * sizeof(KWAIT_BLOCK), 'KWTB');
...
int perfmon_interrupt_centry(void)
{
	...
	{	
		...
		temp = ExAllocatePool2(POOL_FLAG_NON_PAGED, blocksize, 'TEMP');
		if (temp)
		{
...
*/
{
	...
	{
		DS_AREA[cpunr()] = ExAllocatePool2(POOL_FLAG_NON_PAGED, params->DS_AREA_SIZE, 'DSAR');
...
NTSTATUS ultimap(UINT64 cr3, UINT64 dbgctl_msr, int _DS_AREA_SIZE, BOOL savetofile, WCHAR *filename, int handlerCount)
{
	...
	DataBlock = ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(_DataBlock) * MaxDataBlocks, 'DABL');
	DataReadyPointerList = ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(PVOID) * MaxDataBlocks, 'DRPL');
