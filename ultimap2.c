...
NTSTATUS ultimap2_waitForData(ULONG timeout, PULTIMAP2DATAEVENT data)
{
	...
	if (UltimapActive)
	{
		...
		waitblock = ExAllocatePool2(POOL_FLAG_NON_PAGED, Ultimap2CpuCount * sizeof(KWAIT_BLOCK), 'UWTB');
...
{
	return ExAllocatePool2(POOL_FLAG_NON_PAGED, ByteSize, 'MEMB');
}
...
void* setupToPA(PToPA_ENTRY *Header, PVOID *OutputBuffer, PMDL *BufferMDL, PRTL_GENERIC_TABLE *gt, ULONG _BufferSize, int NoPMI)
{
	...
	if (singleToPASystem)
	{
		...
		*OutputBuffer=MmAllocateContiguousMemorySpecifyCache(newsize, la, ha, boundary, MmCached);
		...
		r = ExAllocatePool2(POOL_FLAG_NON_PAGED, 4096, 'RBUF');
		if (r == NULL)
		{
	...
	else
	{
		...
		*OutputBuffer = ExAllocatePool2(POOL_FLAG_NON_PAGED, _BufferSize, 'OBUF');
		if (*OutputBuffer == NULL)
		{
		...
		r = ExAllocatePool2(POOL_FLAG_NON_PAGED, getToPAHeaderSize(_BufferSize), 'TPAH');
		if (r == NULL)
		{
	...
	*gt = ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(RTL_GENERIC_TABLE), 'GTBL');
...
void SetupUltimap2(UINT32 PID, UINT32 BufferSize, WCHAR *Path, int rangeCount, PURANGE Ranges, int NoPMI, int UserMode, int KernelMode)
{
	...
	if (rangeCount)
	{
		...
		Ultimap2Ranges = ExAllocatePool2(POOL_FLAG_NON_PAGED, rangeCount * sizeof(URANGE), 'U2RG');
	...
	PInfo = ExAllocatePool2(POOL_FLAG_NON_PAGED, Ultimap2CpuCount * sizeof(PProcessorInfo), 'PINF');
	Ultimap2_DataReady = ExAllocatePool2(POOL_FLAG_NON_PAGED, Ultimap2CpuCount * sizeof(PVOID), 'U2DR');
	...
	for (i = 0; i < Ultimap2CpuCount; i++)
	{
		PInfo[i] = ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(ProcessorInfo), 'PRCI');
